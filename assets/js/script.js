 ...
 */
const getWeekDayName = function (dayNumer) {
    switch (dayNumer) {
        case 0:
            return "Sunday";
        case 1:
...
/**
 * convert month number to weekday name
 * totalParameter: 1;
...
const taskInputValidation = function (taskIsValid) {
    if (taskIsValid) {
        ...
        // if user press any falsy value like(0, "", undefinde, null, NaN)
        console.log("Pleass write something!")
    }
}
...
const removeWelcomNote = function () {
    if (taskList.childElementCount > 0) {
        welcomeNote.classList.add("hide")
    } else {
    ...
    if (this.dataset.taskRemove === "complete") {
        ...
        taskCompleteSound.play(), // play taskCompleteSound
            ...
            setTimeout(function () {
                ...
                removeWelcomNote(); // remove welcome note 
            }, 250)
...
/**
 * add keypress listener on taskInpt
 */
